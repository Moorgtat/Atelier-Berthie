 ...
 */
final class Version20201123084752 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__commande AS SELECT id, user_id, created_at, transporteur_titre, transporteur_prix, livraison, reference, stripe_session_id, state, suivi FROM commande');
        $this->addSql('DROP TABLE commande');
        $this->addSql('CREATE TABLE commande (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, created_at DATETIME NOT NULL, transporteur_titre VARCHAR(255) NOT NULL COLLATE BINARY, transporteur_prix DOUBLE PRECISION NOT NULL, livraison CLOB NOT NULL COLLATE BINARY, reference VARCHAR(255) NOT NULL COLLATE BINARY, stripe_session_id VARCHAR(255) DEFAULT NULL COLLATE BINARY, state INTEGER NOT NULL, suivi VARCHAR(255) DEFAULT NULL COLLATE BINARY, CONSTRAINT FK_6EEAA67DA76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO commande (id, user_id, created_at, transporteur_titre, transporteur_prix, livraison, reference, stripe_session_id, state, suivi) SELECT id, user_id, created_at, transporteur_titre, transporteur_prix, livraison, reference, stripe_session_id, state, suivi FROM __temp__commande');
        ...
        $this->addSql('DROP INDEX IDX_B9983CE5A76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__reset_password AS SELECT id, user_id, token, created_at FROM reset_password');
        $this->addSql('DROP TABLE reset_password');
        $this->addSql('CREATE TABLE reset_password (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, token VARCHAR(255) NOT NULL COLLATE BINARY, created_at DATETIME NOT NULL, CONSTRAINT FK_B9983CE5A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO reset_password (id, user_id, token, created_at) SELECT id, user_id, token, created_at FROM __temp__reset_password');
        $this->addSql('DROP TABLE __temp__reset_password');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TABLE header (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, titre VARCHAR(255) NOT NULL COLLATE BINARY, content CLOB NOT NULL COLLATE BINARY, url VARCHAR(255) NOT NULL COLLATE BINARY, image VARCHAR(255) NOT NULL COLLATE BINARY)');
        $this->addSql('DROP INDEX IDX_C35F0816A76ED395');
        $this->addSql('CREATE TEMPORARY TABLE __temp__adresse AS SELECT id, user_id, titre, prenom, nom, societe, numero, rue, codepostal, ville, pays, telephone FROM adresse');
        $this->addSql('DROP TABLE adresse');
